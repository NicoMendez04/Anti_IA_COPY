...
function ProfessorDashboard() {
  ...
  useEffect(() => {
    ...
    socket.connect();
    socket.emit('professor:join', { sessionId: session.sessionId });
  ...
  async function createSession(event) {
    ...
    try {
      const response = await fetch(`${API_URL}/api/sessions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
  ...
  async function endSession() {
    await fetch(`${API_URL}/api/sessions/${session.sessionId}`, { method: 'DELETE' });
    setSession((current) => ({ ...current, status: 'ended' }));
  }
  function startSession() {
    socket.emit('professor:start', { sessionId: session.sessionId });
  }
  async function expel(studentId) {
    await fetch(`${API_URL}/api/sessions/${session.sessionId}/expel/${studentId}`, { method: 'POST' });
...
function StudentLive() { const { sessionId } = useParams(); const navigate = useNavigate(); const location = useLocation(); const [session, setSession] = useState(null); const [alertCount, setAlertCount] = useState(0); const [now, setNow] = useState(Date.now()); const [soundEnabled, setSoundEnabled] = useState(false); const audioContext = useRef(null); const name = location.state?.name || 'Estudiante';
  ...
  useEffect(() => { const report = (type, message) => { setAlertCount((value) => value + 1); socket.emit('student:event', { sessionId, type, message }); }; const blur = () => report('window_blur', 'Perdió foco de ventana'); const visibility = () => document.hidden && report('tab_switch', 'Cambió de pestaña'); window.addEventListener('blur', blur); document.addEventListener('visibilitychange', visibility); return () => { window.removeEventListener('blur', blur); document.removeEventListener('visibilitychange', visibility); }; }, [sessionId]);
