...
function useAuth(role) {
  ...
  useEffect(() => { if (!auth?.token) return; fetch(`${API_URL}/api/auth/me`, { headers: { Authorization: `Bearer ${auth.token}` } }).then((r) => { if (!r.ok) throw new Error(); }).catch(() => { setAuth(null); localStorage.removeItem(storageKey); }); }, []);
  function login(nextAuth) { setAuth(nextAuth); localStorage.setItem(storageKey, JSON.stringify(nextAuth)); }
  function logout() { setAuth(null); localStorage.removeItem(storageKey); }
  return { auth, login, logout };
}

function LoginForm({ role, onAuthenticated }) {
  const [email, setEmail] = useState(''); const [password, setPassword] = useState(''); const [error, setError] = useState(''); const [loading, setLoading] = useState(false);
  async function submit(event) { event.preventDefault(); setError(''); setLoading(true); try { const response = await fetch(`${API_URL}/api/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password, role }) }); const data = await response.json(); if (!response.ok) throw new Error(data.message); onAuthenticated(data); } catch (caught) { setError(caught.message || 'No se pudo iniciar sesión.'); } finally { setLoading(false); } }
  return <section className="setup-page"><div className="page-kicker">{role === 'profesor' ? 'ACCESO DE PROFESOR' : 'ACCESO DE ESTUDIANTE'} <span>01</span></div><div className="setup-grid"><div><h1>Inicia con tu<br /><em>correo institucional.</em></h1><p className="lede">Usa tu correo institucional y una contraseña. Si es la primera vez que entras, tu cuenta se crea automáticamente.</p></div><form className="session-form" onSubmit={submit}><label>Correo institucional<input autoFocus required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nombre@institucion.edu" /></label><label>Contraseña<input required type="password" minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Mínimo 6 caracteres" /></label>{error && <p className="error-message">{error}</p>}<button className="button button-dark" type="submit" disabled={loading}>{loading ? 'Verificando…' : 'Continuar'} <span>→</span></button></form></div></section>;
}
...
function StudentLive() { const { sessionId } = useParams(); const navigate = useNavigate(); const location = useLocation(); const { auth } = useAuth('estudiante'); const [session, setSession] = useState(null); const [alertCount, setAlertCount] = useState(0); const [now, setNow] = useState(Date.now()); const [soundEnabled, setSoundEnabled] = useState(false); const [helpStatus, setHelpStatus] = useState('idle'); const audioContext = useRef(null); const name = location.state?.name || 'Estudiante';
  useEffect(() => { if (!auth) navigate(`/student/${sessionId}`); }, [auth, sessionId]);
  useEffect(() => { if (!auth) return; fetch(`${API_URL}/api/sessions/${sessionId}`).then((r) => r.json()).then(setSession); const onConnected = (data) => setSession((current) => ({ ...current, students: data.students })); const onUpdate = (data) => setSession(data); const onExpelled = () => navigate('/student'); const onEnded = () => setSession((current) => ({ ...current, status: 'ended' })); const onAlert = (alert) => { setAlertCount((value) => value + 1); playAlertSound(); showAlertNotification(alert); }; const onHelpResolved = () => setHelpStatus('idle'); socket.on('session:connected', onConnected); socket.on('session:updated', onUpdate); socket.on('student:expelled', onExpelled); socket.on('session:ended', onEnded); socket.on('alert:recorded', onAlert); socket.on('help:resolved', onHelpResolved); socket.connect(); socket.emit('student:join', { sessionId, name, token: auth.token }); if ('Notification' in window && Notification.permission === 'default') Notification.requestPermission(); return () => { socket.off('session:connected', onConnected); socket.off('session:updated', onUpdate); socket.off('student:expelled', onExpelled); socket.off('session:ended', onEnded); socket.off('alert:recorded', onAlert); socket.off('help:resolved', onHelpResolved); socket.disconnect(); }; }, [sessionId, auth?.token]);
