...
function StudentLive() { const { sessionId } = useParams(); const navigate = useNavigate(); const location = useLocation(); const { auth } = useAuth('estudiante'); const [session, setSession] = useState(null); const [alertCount, setAlertCount] = useState(0); const [now, setNow] = useState(Date.now()); const [soundEnabled, setSoundEnabled] = useState(false); const audioContext = useRef(null); const name = location.state?.name || 'Estudiante';
  useEffect(() => { if (!auth) navigate(`/student/${sessionId}`); }, [auth, sessionId]);
  useEffect(() => { if (!auth) return; fetch(`${API_URL}/api/sessions/${sessionId}`).then((r) => r.json()).then(setSession); const onConnected = (data) => setSession((current) => ({ ...current, students: data.students })); const onUpdate = (data) => setSession(data); const onExpelled = () => navigate('/student'); const onEnded = () => setSession((current) => ({ ...current, status: 'ended' })); const onAlert = (alert) => { setAlertCount((value) => value + 1); playAlertSound(); showAlertNotification(alert); }; socket.on('session:connected', onConnected); socket.on('session:updated', onUpdate); socket.on('student:expelled', onExpelled); socket.on('session:ended', onEnded); socket.on('alert:recorded', onAlert); socket.connect(); socket.emit('student:join', { sessionId, name, token: auth.token }); if ('Notification' in window && Notification.permission === 'default') Notification.requestPermission(); return () => { socket.off('session:connected', onConnected); socket.off('session:updated', onUpdate); socket.off('student:expelled', onExpelled); socket.off('session:ended', onEnded); socket.off('alert:recorded', onAlert); socket.disconnect(); }; }, [sessionId, auth?.token]);
